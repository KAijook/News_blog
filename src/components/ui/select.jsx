...
function SelectContent({
  className,
  children,
  position = "popper", // Đổi mặc định thành "popper" để sideOffset hoạt động
  align = "center",
  ...props
}) {
  return (
...
function SelectItem({ className, children, ...props }) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex  w-full cursor-pointer items-center justify-center gap-2.5 rounded-2xl py-[8px] px-[10px] text-sm font-medium outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className,
      )}
      {...props}
    >
